...
class SocketService {
  ...
  void _setupListeners() {
    ...
    _socket!.on('message:new', (data) {
      try {
        final messageData = data is Map<String, dynamic> 
            ? (data['message'] ?? data) 
            : data;
        final message = MessageModel.fromJson(messageData as Map<String, dynamic>);
        for (final listener in _messageListeners) {
          listener(message);
        }
        // Auto send delivered status
        if (message.senderId != _userId) {
          sendDelivered(message.id, message.chatId);
        }
      } catch (e) {
    ...
    _socket!.on('typing:start', (data) {
      ...
      if (chatId != null && uId != null && uId != _userId) {
        for (final listener in _typingStartListeners) {
          listener(chatId, uId, userName);
        }
      }
    });
    ...
    _socket!.on('typing:stop', (data) {
      ...
      if (chatId != null && uId != null) {
        for (final listener in _typingStopListeners) {
          listener(chatId, uId);
        }
      }
    });
    ...
    _socket!.on('call:incoming', (data) {
      _notifyCall({...data as Map<String, dynamic>, 'event': 'incoming'});
    });
    _socket!.on('call:accepted', (data) {
      _notifyCall({...data as Map<String, dynamic>, 'event': 'accepted'});
    });
    _socket!.on('call:rejected', (data) {
      _notifyCall({...data as Map<String, dynamic>, 'event': 'rejected'});
    });
    _socket!.on('call:ended', (data) {
      _notifyCall({...data as Map<String, dynamic>, 'event': 'ended'});
    });
    _socket!.on('call:signal', (data) {
      _notifyCall({...data as Map<String, dynamic>, 'event': 'signal'});
    });
    _socket!.on('call:ice-candidate', (data) {
      _notifyCall({...data as Map<String, dynamic>, 'event': 'ice-candidate'});
    });
  }
  ...
  void onMessageStatus(Function(String, String, String?) callback) {
    _statusListeners.add(callback);
  }

  void onTypingStart(Function(String, String, String) callback) {
    _typingStartListeners.add(callback);
  }

  void onTypingStop(Function(String, String) callback) {
    _typingStopListeners.add(callback);
  }

  void onPresenceUpdate(Function(String, String, String?) callback) {
  ...
  void sendDelivered(String messageId, String chatId) {
  ...
  void sendRead(String messageId, String chatId) {
